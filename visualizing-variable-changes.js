(function(){
  ...
  $(document).ready(() => {
    ...
    $('#forward').click((e) => {
      ...
      varsNoLongerOnWatchList(declaredVariables, watchedVariables)
    });

    $('#back').click(() => {
      var watchedVariablesObj,
          decVar,
          lineNumber = data[indexCounter].line ;

      if (indexCounter > -1) {
        removeLineIndicator(lineNumber + 1);
        indexCounter--;
        disableForwardLastButtons(false);
        $('#step').text(`Step ${indexCounter + 1} of ${dataLength}`);
        placeLineIndicator(lineNumber + 1);
      }
  ...
  function varsNoLongerOnWatchList(declaredVariables, watchedVariables) {
    for (var i = 0, n = declaredVariables.length; i < n; i ++) {
      console.log("MAKE IT UNDEFINED");
