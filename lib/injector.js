...
function addVars(lineInfo) {
  ...
  console.log('special varvar', specialVariables);
}
...
function checkClosingLine(lineInfo, lineText) {
  if (lineInfo) {
    ...
    //if a line is a opening or closing brace - what if symbol
...
function findGlobalVars(lineRules, line, index) {
  if (injectCheck[injectCheck.length - 1]) {
    ...
    if (specialVariables[0].length >= 1 || specialVariables.length > 1) {
      ...
      console.log('SLICE', specialVariables.slice(0), specialVariables)
      console.log('specialVarLog', specialVariablesLog);
    }
  }
}
/**
...
console.log('SCOPE', scope);
  ...
  else globalVariables = undefined;
...
function injectionEngine(rules, text) {
  ...
  textArray.forEach((line, i) => {
    ...
    findScope(rules[i]);
    addVars(rules[i]);
    // console.log('injection log', injectionLog);
