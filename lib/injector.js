...
function injector(log, line, index, check, lineRules, closingLine) {
  ...
  mintyLine = `mintyLog(${index}, '${currentScope}', ${JSON.stringify(lineSpecial)}, ${JSON.stringify(variables)}`;
  ...
  if (!check || closingLine || line === '') log.push(`${line}`);
  else if (returnStatement) log.push(mintyLine, `${line}`, mintyLine);
