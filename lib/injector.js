...
function concatVariables(lineRules, line, index) {
...
function injector(log, line, index, check) {
  let currentScope;
  scope.forEach(block => {
    if (block) currentScope = block;
  });
...
function injectionEngine(rules, text) {
  ...
  textArray.forEach((line, i) => {
    if (line !== '') {
      concatVariables(rules[i], line, i);
    }
  });
  textArray.forEach((line, i) => {
    ...
    findScope(rules[i]);
    addVars(rules[i]);
    injector(injectionLog.log, line, i, injectCheck[injectCheck.length - 1]);
  });
