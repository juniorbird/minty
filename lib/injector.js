...
function findScope(lineInfo) {
  ...
  lineInfo.rulesFound.forEach(rule => {
    // console.log('rule rule', rule.rule, rule)
    if (rule.rule === 'SCOPE' && rule.action === 'START') {
      if (lineInfo.scope) {
        // console.log('line scope', lineInfo.scope)
...
* special variables log
* @param {object} lineRules - rules created for each corresponding line of code user provides
...
function findGlobalVars(lineRules, line, index) {
  if (injectCheck[injectCheck.length - 1]) {
    ...
    if (specialVariables[0].length >= 1 || specialVariables.length > 1) {
      //not global variable - keeps track of what the special variables are supposed to be
      specialVariablesLog[index] = specialVariables.slice(0);
      console.log(specialVariablesLog);
    }
  }
}
/**
...
function injectionEngine(rules, text) {
  ...
  specialVariables = [[]];
  variables = [];
  // console.log('INJECTION enging', text);
  //EOL end of line - determines line carriage because of different operating system (\n, \n\r)
