  ...
  function variableAdd(type, key) {
    ...
    if (key === 'special') {
      for (let el in type) {
        ...
        type[el].forEach(val => {
          retObj.variables[key][el][val] = args[0];
          args.splice(0, 1);
        });
      }
    } else {
      while (array.length) {
        retObj.variables[key][array[0]] = args[0];
        args.splice(0, 1);
        array.splice(0, 1);
      }
    }
...
function finalizeRun(output, type) {
  ...
  fs.mkdir(appRoot + '/minty', (err) => {
    fs.mkdir(appRoot + `/minty/${type}`, (err2) => {
      const jquery = fs.readFileSync(appRoot + '/node_modules/minty/node_modules/jquery/dist/jquery.min.js');
      ...
      const insert = `${beginningHtml} \n <script type="text/javascript"> \n ${jquery} \n </script> \n <style>`
        ...
        + `<script>${viz}</script>\n`
        + `<script type="text/javascript"> \n var data = ${output} \n ${template} \n </script> \n ${endHtml}`;
      fs.writeFile(appRoot + `/minty/${type}/${name}_${date.toString().replace(/\s+/gm, '_').replace(/:+/gm, '-')}.html`, insert, (err11) => {});
    });
  });
}
