...
const types = [
  {
    ...
    callbacks: [[parseutils.variableKindParse, 'kind'], [parseutils.variableNameParse, 'variables']],
  },
  ...
  { type: 'YieldExpression', callbacks: null },
  {
    ...
    callbacks: [[parseutils.functionParameterParse, 'parameters'], [parseutils.functionNameParse, 'name']],
  },
  {
  ...
  { type: 'AssignmentExpression', callbacks: [[parseutils.assignmentExpParse, 'name']] },
];
  ...
  output.forEach(node => {
    ...
    if (callbacks) {
      callbacks.forEach(cbArray => {
        ...
        parseutils.cache[searchString][parseutils.cache[searchString].length - 1][name] = extraOutput;
      });
    }
  });
}
...
module.exports = {
  ...
  tester: {
    types: types,
    query: parseutils.query,
    functionParameterParse: parseutils.functionParameterParse,
    variableKindParse: parseutils.variableKindParse,
    variableNameParse: parseutils.variableNameParse,
    parseFunction: parseutils.parseFunction,
    asyncTasks: parseutils.asyncTasks,
  },
};
