...
function addLines(type, rule, cluster) {
  cluster.forEach(group => {
    if (!lineActivity[group.startLine - 1]) {
      lineActivity[group.startLine - 1] = {};
    }

    if (!lineActivity[group.startLine - 1].rulesFound) {
      lineActivity[group.startLine - 1].rulesFound = [];
    }
    ...
    if (!lineActivity[group.endLine - 1]) {
      lineActivity[group.endLine - 1] = {};
    }

    if (!lineActivity[group.endLine - 1].rulesFound) {
      lineActivity[group.endLine - 1].rulesFound = [];
    }
