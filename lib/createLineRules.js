...
function addLines(type, rule, cluster, lineActivity) {
  ...
  console.log('lineaca', util.inspect(lineActivity, false, null));
}
...
function ruler(parsed) {
  ...
  Object.keys(parsed).forEach((type, i) => {
    ...
    switch (type) {
      ...
      case 'YieldExpression':
        addLines(type, 'SWAP', parsed[type], lineActivity);
        break;
      case 'FunctionDeclaration':
      case 'FunctionExpression':
      case 'ArrowFunctionExpression':
      case 'SwitchCase':
        addLines(type, 'SCOPE', parsed[type], lineActivity);
        addVariables(parsed[type], lineActivity);
        addScopeName(parsed[type], lineActivity);
        break;
      case 'VariableDeclaration':
        addLines(type, null, parsed[type], lineActivity);
        addVariables(parsed[type], lineActivity);
        break;
      case 'CallExpression':
      case 'SwitchStatement':
        addLines(type, 'VOID', parsed[type], lineActivity);
        break;
      default:
        addLines(type, 'SCOPE', parsed[type], lineActivity);
        break;
  ...
  // console.log(util.inspect(lineActivity, false, null));
