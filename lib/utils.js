...
minty.splitFile = function splitFile(text, file, callback) {
  ...
  let name = file;
  modifiedText = decomment(modifiedText, {
    space: true
  });
  const EOL = decomment.getEOL(modifiedText);
  modifiedText = modifiedText.split(EOL);
  if (!name) {
    name = modifiedText[0].match(/(?=function)(.*)(?=\()/gm, '')
      .join('')
      .replace(/function/gm, '')
      .trim();
    if (!name) name = 'anonymous';
  }
  modifiedText = minty.multiLineVarsCheck(modifiedText);
  if (callback) callback(null, modifiedText, name);
  else {
    return minty.injectCheckCode(modifiedText, name);
  }
};
