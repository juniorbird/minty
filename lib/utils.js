...
minty.splitFile = function splitFile(results, file, callback) {
  ...
  results = minty.multiLineVarsCheck(results);
  callback(null, results, file);
};
...
minty.multiLineVarsCheck = function (arrayOfResults) {
  ...
  arrayOfResults.forEach(function (element, index, array) {
    ...
    // If we're on a line that matches our strings
    stringMatch = element.match(/".*"/);
    regexMatch = element.match(\/.*\/);

    console.log('strings:', stringMatch, 'regexes', regexMatch);


    // varsOnThisLine = minty.findVariables(element);
    // if (varsOnThisLine && (varsOnThisLine.constructor === Array)) {
      console.log('line#', index + 1, element);
      console.log('foundVariables', index, minty.findVariables(element));
      // Get the array of opening and closing parens
      thisLineOpeners = element.match(/\{|\[/g);
      if (thisLineOpeners) thisLineOpeners.reverse();
      thisLineClosers = element.match(/\}|\]/g);

      // console.log('start', thisLineOpeners, thisLineClosers);

      // See if we've closed all our open parens
      if (thisLineClosers) {
        thisLineClosers.forEach(function (element) {
          if (element === ']') {
            if (thisLineOpeners && (thisLineOpeners[0] === '[')) thisLineOpeners.shift();
          } else if (element === '}') {
            if (thisLineOpeners && (thisLineOpeners[0] === '{')) thisLineOpeners.shift();
          } else {
            // this is an error with an unbalanced set of parens. What do we do here?
          }
        });
        ...
        if (thisLineClosers.length > 0) {
          thisLineClosers.forEach(function (element) {
            if (element === ']') {
              if (openers && (openers[0] === '[')) openers.shift();
            } else if (element === '}') {
              if (openers && (openers[0] === '{')) openers.shift();
            } else {
              // this is an error with an unbalanced set of parens. What do we do here?
      ...
      if (openers.length > 0) {
        ...
        concatStart = index - linesToConcat; // -1 because we count the current line
        console.log('start concat from line', concatStart, 'stopping at', index + 2);
        for (var i = concatStart + 1; i < index + 2; i++) {
      ...
      console.log('lines to concat', linesToConcat);

      console.log('openers', openers);

    // }

    // varsOnThisLine.forEach(function (element, index, array) {
      // Then, if we're looking at an array or an object
      //  If the last character of the line isn't the closing character e.g. ] or }
      //    Delete the linebreak at the beginning of the line
      //    Increment our linebreak counter
      //    When we find the closing character, place that many line breaks
        // console.log('this var:', element);
    // });
  });
