...
minty.multiLineVarsCheck = function (arrayOfResults) {
  ...
  arrayOfResults.forEach(function (thisLine, index, array) {

    // Make a dupe of this line to do destructive operations on
    thisLine = JSON.parse(JSON.stringify(thisLine)); // makes a copy no matter what the content is

    startOpeners = openers.length;

    console.log('line#', index + 1, thisLine);
    console.log('foundVariables', index, minty.findVariables(thisLine));
