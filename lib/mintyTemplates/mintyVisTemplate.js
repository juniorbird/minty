$(document).ready(() => {
  ...
  function changeVars() {
    ...
    for(let keys in globalVars) {
      const value = globalVars[keys] === '_mintyUndefined' ? undefined : globalVars[keys]
      $('#watch-list').append(`<tr><td>${keys}</td><td>${JSON.stringify(value)}</td></tr>`);
    }
    for(let keys in specialVars) {
      const value = specialVars[keys] === '_mintyUndefined' ? undefined : specialVars[keys]
      $('#watch-list').append(`<tr><td>${keys}</td><td>${JSON.stringify(value)}</td></tr>`);
    }
  }
  ...
  $('#forward').click(() => {
    currentIndex++;
    prevLine = currLine;
    currLine = data.log[currentIndex].line;
    globalVars = data.log[currentIndex].variables.global;
    specialVars = data.log[currentIndex].variables.special
    console.log(data.log[currentIndex])
    changeVars();
    moveLineIndicator(currentIndex, currentIndex - 1, data.log)
  });
...
//       globalVars[variables] = undefined;
//     }
//   });
...
//   }
//
//   declaredVariables = Object.keys(globalVars);
//
//
...
//       appendReturnValue();
//
//     //initially this globalVars's variable values were all undefined
...
//       globalVars[variable] = variablesLastIndex[variable];
//     });
//
//     $('.variables').each((i, elem) => {
//       variable = globalVars[declaredVariables[i]];
