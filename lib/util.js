...
const types = [
  ...
  { type: 'VariableDeclaration', callbacks: [[variableParse, 'parameters']] },
];
...
function variableParse() {
  const variableDeclaration = query('[type="VariableDeclaration"]', ast);
  const varDeclarator = query('[type="VariableDeclarator"]', ast);
  const cacheVar = cache.VariableDeclaration;
  var obj;
  // console.log(allVariableDeclarions)

  variableDeclaration.forEach(node => {
    obj = {}
    obj.startLine = node.loc.start.line
    obj.endLine = node.loc.end.line
    obj.kind = node.kind
    cacheVar.push(obj);
  });

  var counter = 0;
  while(counter < cacheVar.length - 1) {
    varDeclarator.forEach(node => {
      if (!cacheVar[counter].variables)
        cacheVar[counter].variables = [];
      if (cacheVar[counter].startLine === node.loc.start.line) {
        cacheVar[counter].variables.push(node.id.name);
      } else {
        counter ++;
        if (!cacheVar[counter].variables)
          cacheVar[counter].variables = [];
